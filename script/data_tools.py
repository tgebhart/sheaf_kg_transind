...
def generate_mapped_triples_both(
    query_loc_hard,
    answer_loc_easy,
    answer_loc_hard,
    ...
    for query_structure in query_structures:
        ...
        qs = hard_queries[
            name_query_dict[query_structure]
        ]  # same for easy and hard queries
        ...
        if filter_fun is not None:
            print(
                f"filtered {num_filtered} queries of {len(qs)} possible for query {query_structure}"
            )
...
def load_queries_and_answers(
    ...
    test_queries = generate_mapped_triples_both(
        dstf[eval_graph_name]["queries"],
        dstf[eval_graph_name]["answers"]["easy"],
        dstf[eval_graph_name]["answers"]["hard"],
        query_structures=query_structures,
        remap_fun=remap_fun,
        skip_ea=skip_ea,
    )
...
def get_train_eval_inclusion_data(
    ...
    if include_complex:
        ...
        r["complex"] = load_queries_and_answers(
            dataset,
            dataset_pct,
            eval_graph,
            eval_graph_type,
...
def load_hpo_config(hpo_config_name: str, load: bool = True) -> dict:
    hpo_config_fname = (
        hpo_config_name if ".json" in hpo_config_name else f"{hpo_config_name}.json"
    )
...
def get_disjoint_dataset(dataset, version, create_inverse_triples=False):
    if dataset == "InductiveFB15k237":
        return InductiveFB15k237(
            version=version, create_inverse_triples=create_inverse_triples
        )
    if dataset == "InductiveWN18RR":
        return InductiveWN18RR(
            version=version, create_inverse_triples=create_inverse_triples
        )
    if dataset == "InductiveNELL":
        return InductiveNELL(
            version=version, create_inverse_triples=create_inverse_triples
        )
