...
def measure_evaluation(
    extender, evaluator, query_structure, queries, evaluation_batch_size
):
    with torch.no_grad():
...
def run(
    ...
    rdata = get_train_eval_inclusion_data(
        dataset, dataset_pct, graph, graph, include_complex=True, skip_ea=True
    )
    ...
    for query_structure in query_structures:
        ...
        result_df = measure_evaluation(
            extender, evaluator, query_structure, queries, evaluation_batch_size
        )
        result_df["epoch"] = 0
        print(
            result_df.set_index(["Side", "Type", "Metric", "epoch"]).loc[
                "tail", "realistic", "hits_at_10", 0
            ]
        )
        ...
        for epoch in epochs:
            ...
            for bix in tqdm(
                range(0, len(queries), training_batch_size),
                ...
                scores = extender.slice_and_score_complex(
                    query_structure, batch_queries, training_batch_size, progress=False
                )
            ...
            if epoch % evaluate_every == 0:
                ...
                print(
                    result_df.set_index(["Side", "Type", "Metric", "epoch"]).loc[
                        "tail", "realistic", "hits_at_10", epoch
                    ]
                )
                results.append(result_df)
