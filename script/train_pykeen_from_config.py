...
def create_neg_batch_list(queries, num_neg_per_pos, entities):
    ...
    return [
        torch.LongTensor(
            np.random.choice(
                list(ent_range.difference(set(q["hard"]))),
                int(num_neg_per_pos * len(q["hard"])),
                replace=True,
            )
        )
        for q in queries
...
def create_sampler_batch(queries):
    return torch.LongTensor(
        [[q["sources"][0], q["relations"][0], a] for q in queries for a in q["hard"]]
    )
...
def attempt_infer_best_hyperparams(best_hpo_loc):
    with open(best_hpo_loc, "r") as f:
        config = json.load(f)
    r = {"margin": None, "num_negs_per_pos": None, "lr": None}
    if "pipeline" in config:
        ...
        if "negative_sampler_kwargs" in pc and pc["negative_sampler"] == "basic":
            r["num_negs_per_pos"] = pc["negative_sampler_kwargs"].get(
...
def train_complex_loop(
    model,
    train_model,
    rdata,
    best_hpo_loc,
    savedir,
    query_structures=TRAINING_QUERY_STRUCTURES,
    complex_epochs=COMPLEX_EPOCHS,
        ...
        for query_structure in query_structures:
            ...
            for bix in tqdm(
                ...
                scores = extender.slice_and_score_complex(
                    query_structure, batch_queries, complex_batch_size, progress=False
                )
                neg_scores = extender.slice_and_score_complex(
                    query_structure,
                    batch_neg_queries,
                    complex_batch_size,
                    progress=False,
                ...
                row_indices = torch.cat(
                    [
                        torch.LongTensor([i] * len(c))
                        for i, c in enumerate(this_batch_list)
                    ]
                )
                hard_scores = scores[row_indices, col_indices].unsqueeze(-1)

                neg_col_indices = torch.cat(this_neg_batch_list)
                neg_row_indices = torch.cat(
                    [
                        torch.LongTensor([i] * len(c))
                        for i, c in enumerate(this_neg_batch_list)
                    ]
                )
                hard_neg_scores = neg_scores[neg_row_indices, neg_col_indices].reshape(
                    hard_scores.shape[0], -1
                )
...
def run(
    ...
    tdata = get_train_eval_inclusion_data(
        dataset, dataset_pct, graph, graph, include_complex=train_complex, skip_ea=True
    )
    print("complete.")
    training_set = tdata["orig"]["triples"]
    testing_set = tdata["eval"]["triples"]
    savedir = (
        f"data/{dataset}/{dataset_pct}/models/development/{graph}/{model}/{config_name}"
    )
...
if __name__ == "__main__":
    ...
    training_args.add_argument(
        "--query_structures", type=str, nargs="+", help="Query structures"
    )
    training_args.add_argument(
        "--complex_epochs",
        type=int,
        default=COMPLEX_EPOCHS,
        help="training epochs for complex",
    )
    training_args.add_argument(
        "--complex_batch_size",
        type=int,
        default=COMPLEX_BATCH_SIZE,
        help="Complex training batch size",
    )
    training_args.add_argument(
        "--train_complex", action="store_true", help="whether to run complex training"
    )
    ...
    run(
        ...
        query_structures=args.query_structures,
