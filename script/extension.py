...
def diffuse_interior(diffuser, triples, interior_ent_msk):
    ...
    xU = diffuser.diffuse_interior(
        edge_index, relations, interior_vertices, nv=num_nodes
    )
    return xU
...
class SEExtender(KGExtender):
    ...
    def diffuse_interior(
        ...
        edge_index: torch.LongTensor,
        edge_type: torch.LongTensor,
        interior_vertices: torch.LongTensor,
        relabel: bool = False,
        nv: int = None,
    ):
        xU = self.laplacian_mult(edge_index, edge_type, relabel=relabel, nv=nv)
        self.model.entity_representations[0]._embeddings.weight[interior_vertices] -= (
            self.alpha * xU[interior_vertices]
        )
        return xU
    ...
    def harmonic_extension(
        ...
        # TODO
        LUU = self.laplacian_block_dense(
            edge_index[:, interior_mask], edge_type[:, interior_mask]
        )
        LUU_inv = torch.linalg.pinv(LUU)
        LUB = self.laplacian_block_dense(
...
class TransEExtender(KGExtender):
    ...
    def diffuse_interior(
        ...
        xU = self.laplacian_mult_translational(
...
class RotatEExtender(KGExtender):
    ...
    def diffuse_interior(
        ...
        self.model.entity_representations[0]._embeddings.weight[
            interior_vertices
        ] -= torch.view_as_real(self.alpha * xU[interior_vertices]).reshape(
            (interior_vertices.shape[0], -1)
        )
        return xU
