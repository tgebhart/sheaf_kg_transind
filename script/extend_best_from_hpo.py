...
def run(
    ...
    orig_savedir = (
        f"data/{dataset}/{dataset_pct}/models/{orig_graph_type}/{model}/hpo_best"
    )
    eval_savedir = (
        f"data/{dataset}/{dataset_pct}/models/{eval_graph_type}/{model}/hpo_best"
    )
    ...
    with torch.no_grad():
        ...
        orig_model, interior_mask, diffusion_fun = expand_model_to_inductive_graph(
