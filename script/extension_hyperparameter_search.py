...
def objective(config, rdata, model_name, complex_query_targets, trial):

    model_kwargs_ranges = config['pipeline'].pop('model_kwargs_ranges', {})
    loss_kwargs_ranges = config['pipeline'].pop('loss_kwargs_ranges', {})
    optimizer_kwargs_ranges = config['pipeline'].pop('optimizer_kwargs_ranges', {})
    negative_sampler_kwargs_ranges = config['pipeline'].pop('negative_sampler_kwargs_ranges', {})
    diffusion_kwargs_ranges = config['extension'].pop('diffusion_ranges', {})
    ...
    with torch.no_grad():
        ...
        for dit in range(diffusion_iterations):
