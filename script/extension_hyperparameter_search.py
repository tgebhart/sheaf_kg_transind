...
def evaluate_complex(
    ...
    model_name,
    rdata,
    query_structures=COMPLEX_QUERY_TARGETS,
    ...
    for query_structure in query_structures:
        ...
        scores = extender.slice_and_score_complex(
            query_structure, queries, evaluation_batch_size
        )
        ...
        res = (
            result_df.set_index(["Side", "Type", "Metric"])
            .loc["tail", "realistic", "hits_at_10"]
            .values[0]
        )
        results.append(res)
        evaluator.clear()
...
def objective(
    orig_config, rdata, model_name, complex_query_targets, diffusion_batch_size, trial
):
    ...
    negative_sampler_kwargs_ranges = config["pipeline"].get(
        "negative_sampler_kwargs_ranges", {}
    )
    diffusion_kwargs_ranges = config["extension"].get("diffusion_ranges", {})

    model_kwargs = {
        key: suggest_value(trial, model_kwargs_ranges[key], key)
        for key in model_kwargs_ranges
    }
    loss_kwargs = {
        key: suggest_value(trial, loss_kwargs_ranges[key], key)
        for key in loss_kwargs_ranges
    }
    optimizer_kwargs = {
        key: suggest_value(trial, optimizer_kwargs_ranges[key], key)
        for key in optimizer_kwargs_ranges
    }
    negative_sampler_kwargs = {
        key: suggest_value(trial, negative_sampler_kwargs_ranges[key], key)
        for key in negative_sampler_kwargs_ranges
    }
    diffusion_kwargs = {
        key: suggest_value(trial, diffusion_kwargs_ranges[key], key)
        for key in diffusion_kwargs_ranges
    }
    ...
    config["pipeline"]["optimizer_kwargs"] = config["pipeline"].get(
        "optimizer_kwargs", {}
    )
    config["pipeline"]["optimizer_kwargs"].update(optimizer_kwargs)

    config["pipeline"]["negative_sampler_kwargs"] = config["pipeline"].get(
        "negative_sampler_kwargs", {}
    )
    ...
    pipeline_config["pipeline"] = {
        k: v for k, v in pipeline_config["pipeline"].items() if "_ranges" not in k
    }
    ...
    with torch.no_grad():
        ...
        model, interior_mask = expand_model_to_inductive_graph(
            model, orig_eval_entity_inclusion, eval_graph
        )
        ...
        evaluation_metric = evaluate_complex(
            extender.model, model_name, rdata, query_structures=complex_query_targets
        )
...
def run(
    ...
    rdata = get_train_eval_inclusion_data(
        dataset, dataset_pct, train_graph, eval_graph, include_complex=True
    )
    ...
    best_config["pipeline"] = {
        k: v
        for k, v in best_config["pipeline"].items()
        if (k not in ("training", "validation", "testing")) and ("_ranges" not in k)
    }
