...
class ComplexExtender(ABC):
    ...
    def __init__(self, model: ERModel, dtype: torch.Type = torch.float32) -> None:
        self.model = model
    ...
    def score_complex(
        self, query_structure: str, queries: List[dict], tails: torch.Tensor
    ) -> torch.Tensor:
    ...
    @abstractmethod
    def score_2p(
        self, queries: List[dict], tails: torch.LongTensor
    ) -> torch.FloatTensor:
        pass
    ...
    def unpack_intersection_query_indices(
    ...
    def slice_and_score_complex(
        ...
        for bix in tqdm(
            ...
            desc=f"batched {query_structure}",
...
class SEComplex(ComplexExtender):
    ...
    def score_intersect_batched(
        self, edge_index, restriction_maps, source_vertices, target_vertices, xS, xT, dv
    ):
    ...
    def score_2p(
        ...
        scores = self.score_schur_batched(
            qs["edge_index"],
            restriction_maps,
            qs["boundary_vertices"],
            qs["interior_vertices"],
            qs["source_vertices"],
            qs["target_vertices"],
            h.reshape(nbatch, -1, 1),
            t,
            t.shape[-2],
        )
    ...
    def score_2i(
        ...
        scores = self.score_intersect_batched(
            qs["edge_index"],
            restriction_maps,
            qs["source_vertices"],
            qs["target_vertices"],
...
class TransEComplexVerbose(ComplexExtender):
    ...
    def score_schur_batched(
        ...
        LSchur, affine = bhe.Kron_reduction_translational(
            edge_index, restriction_maps, boundary_vertices, interior_vertices
        )
        return bhe.compute_costs_translational(
            LSchur, affine, source_vertices, target_vertices, xS, xT, b, dv
        )
    ...
    def score_intersect_batched(
        ...
        return bhe.compute_costs_translational(
            L, d, source_vertices, target_vertices, xS, xT, b, dv
        )
...
class RotatEComplex(ComplexExtender):
    ...
    def score_2i(
        ...
        return self.model.interaction.score(
            h=hnew, r=torch.ones_like(hnew, dtype=self.dtype), t=t
        )
