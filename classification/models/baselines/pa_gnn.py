...
class PaGNNConv(torch.nn.Module):
    def __init__(self, in_features, out_features, mask, edge_index):
        ...
        self.adj = torch.sparse.FloatTensor(edge_index, values=edge_weight).to(
            edge_index.device
        )

    def forward(self, x, edge_index):
        x[x.isnan()] = 0
        numerator = torch.sparse.mm(self.adj, torch.ones_like(x)) * torch.sparse.mm(
            self.adj, self.mask * x
        )
