...
class KnowledgeSheaf(torch.nn.Module):
    ...
    def __init__(self, n_nodes, edge_index, entity_types, stalk_dim: int, restriction_maps = None, orthogonal = True, verbose = False, device = None):
        super().__init__()
    ...
    def sheaf_dirichlet_energy(self, entity_reps):
        ...
        head_maps = normalized_maps[labeled_edge_index[0],labeled_edge_index[1]].reshape(-1,self.stalk_dim,self.stalk_dim)
        tail_maps = normalized_maps[labeled_edge_index[1], labeled_edge_index[0]].reshape(-1,self.stalk_dim,self.stalk_dim)
    ...
    def train_maps(self, entity_reps, epochs = 1, lr = 0.1):
        ...
        if self.orthogonal:
            ortho_maps = torch.tensor([torch_householder_orgqr(torch.rand(self.stalk_dim,self.stalk_dim).tril(diagonal = -1) + torch.eye(self.stalk_dim,self.stalk_dim)).tolist() for _ in range(self.n_entity_types*self.n_entity_types)]).reshape(self.n_entity_types, self.n_entity_types, self.stalk_dim, self.stalk_dim)
