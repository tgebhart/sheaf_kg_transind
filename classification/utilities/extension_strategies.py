...
def neighborhood_mean_filling(edge_index, X, feature_mask):
    ...
    D = torch.sparse.spmm(
        edge_index_mm, edge_values, n_nodes, n_nodes, feature_mask.float()
    )
    mean_neighborhood_features = (
        torch.sparse.spmm(edge_index_mm, edge_values, n_nodes, n_nodes, X_zero_filled)
        / D
    )
...
def feature_propagation(edge_index, X, Y, feature_mask, num_iterations, sheaf: bool):
    ...
    if sheaf == False:
        ...
        propagation_mat = torch.sparse.FloatTensor(
            edge_index, values=edge_weight, size=(n_nodes, n_nodes)
        ).to(edge_index.device)
    else:
        edge_index, edge_blocks = sheaf_diffusion_iter(
            X, Y, edge_index, n_nodes=n_nodes
        )  # First edge labels are 2-->6

        print(
            "matrix norms of laplacian blocks: {}".format(
                torch.linalg.matrix_norm(edge_blocks)
            )
        )
        ...
        crow_indices = torch.tensor(
            csr_array(
                (torch.ones(n_edges), (edge_index[0].cpu(), edge_index[1].cpu())),
                shape=(n_nodes, n_nodes),
            ).indptr
        )  # This is a hacky way to create the compressed row index format....
        col_indices = edge_index[1]
        propagation_mat = torch.sparse_bsr_tensor(
            crow_indices,
            col_indices,
            edge_blocks,
            size=(n_nodes * 2, n_nodes * 2),
            device=edge_index.device,
        )

        # Oh yeah, and I forgot to implement I-Delta in the previous step.

        propagation_mat = torch.sparse.addmm(
            input=torch.eye(n_nodes * 2).to(propagation_mat.device),
            mat1=propagation_mat,
            mat2=torch.eye(n_nodes * 2).to(propagation_mat.device),
            alpha=-1,
        )

        X = X.reshape(
            (-1, 1)
        )  # Change a [n_nodes, num_features] to [n_nodes*n_features,1] so I can multiply it by propagation_mat
    ...
    return propagation_model.propagate(
        x=X, mask=feature_mask, propagation_mat=propagation_mat
    )
...
def mixhop_rotation_matrix(c, nclass=10):
    ...
    s = torch.sin(angle)
    c = torch.cos(angle)
    rot = torch.stack([torch.stack([c, -s], dim=1), torch.stack([s, c], dim=1)], dim=1)
    # return torch.tensor([[c,-s],[s,c]], device=c1.device)
    return rot
...
def lap_mult(edge_index, Fh, Ft, xh, xt, nv=None, degree_normalize=False):
    if degree_normalize:
        ...
        D = torch.inverse(D.pow(0.5))
        Fh = Fh @ D[edge_index[0]]
        Ft = Ft @ D[edge_index[1]]
        dx = Fh @ D[edge_index[0]] @ xh.unsqueeze(-1) - Ft @ D[
            edge_index[1]
        ] @ xt.unsqueeze(-1)
...
def mixhop_exact_restriction_propagation(
    edge_index, X, Y, feature_mask, num_iterations, alpha=0.1
):
    ...
    for _ in tqdm(range(num_iterations), desc="diffusion"):
        ...
        xt = x[edge_index[1, :]]
        x -= alpha * lap_mult(
            edge_index, Fh, Ft, xh, xt, nv=X.shape[0], degree_normalize=True
        )
...
def filling(filling_method, edge_index, X, Y, feature_mask, num_iterations=None):
    if filling_method == "random":
        ...
        X_reconstructed = feature_propagation(
            edge_index, X, Y, feature_mask, num_iterations, sheaf=True
        )
        X_reconstructed = X_reconstructed.reshape(X.shape)
    elif filling_method == "constant_propagation":
        X_reconstructed = feature_propagation(
            edge_index, X, Y, feature_mask, num_iterations, sheaf=False
        )
    elif filling_method == "mixhop_exact_restriction":
        X_reconstructed = mixhop_exact_restriction_propagation(
            edge_index, X, Y, feature_mask, num_iterations
        )
    else:
