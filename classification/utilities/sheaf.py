...
class SheafLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        stalk_dim: int,
        n_edge_attr: int,
        matrix_weighted=False,
        signtype: str = None,
        signdiag: bool = False,
        layertype: str = "Laplacian",
        euler_stepsize: float = 0.1,
        train_restriction_maps: bool = True,
        ...
        **kwargs
        ...
        if self.matrix_weighted:
            restriction_maps = (
                torch.randn((n_edge_attr, stalk_dim, stalk_dim)) / stalk_dim
            )  # this normalization means with high probability restriction map norms are <= 1.
        else:
            restriction_maps = (
                torch.randn((n_edge_attr, 2, stalk_dim, stalk_dim)) / stalk_dim
            )
    ...
    def _restriction_maps(self, edge_attr: torch.Tensor) -> torch.FloatTensor:
        ...
        if edge_attr.dtype == torch.float:
            if self.matrix_weighted:
                F = (
                    edge_attr
                    @ self.restriction_maps.reshape(
                        (-1, self.stalk_dim * self.stalk_dim)
                    )
                ...
                F = (
                    edge_attr
                    @ self.restriction_maps.reshape(
                        (-1, 2 * self.stalk_dim * self.stalk_dim)
                    )
                ).reshape((-1, 2, self.stalk_dim, self.stalk_dim))
                F_left = F[:, 0, :, :]
                F_right = F[:, 1, :, :]
    ...
    def _edge_weights(self, edge_attr: torch.Tensor) -> torch.FloatTensor:
        if self.signed:
            ...
            edge_weights = (
                edge_attr @ self.signs.reshape((self.n_edge_features, -1))
            ).reshape((n_edges, -1, 1))
        else:
    ...
    def universal_coboundary(
        ...
        if self.matrix_weighted:
            if self.signtype == "sheaf":
                # raise NotImplementedError()
                x_left = torch.abs(self.signs.unsqueeze(1)) * (
                    self.restriction_maps.unsqueeze(1) @ x_left.unsqueeze(0)
                )
                x_right = self.signs.unsqueeze(1) * (
                    self.restriction_maps.unsqueeze(1) @ x_right.unsqueeze(0)
                )
                dx = x_left - x_right
            else:
                dx = self.restriction_maps.unsqueeze(1) @ (x_left - x_right).unsqueeze(
                    0
                )
        else:
            x_e = torch.cat(
                (x_left.unsqueeze(1), -x_right.unsqueeze(1)), dim=1
            )  # this makes the sum calculate the correctly signed coboundary. x_e is (edges, 2, stalkdim, features)
            x_e = self.restriction_maps.unsqueeze(1) @ x_e.unsqueeze(
                0
            )  # (edge_attr, 1, 2, stalkdim, stalkdim) @ (1, edges, 2, stalkdim, features) = (edge_attr, edges, 2, stalkdim, features)
            if self.signtype == "sheaf":
                weights = torch.cat(
                    (torch.abs(self.signs.unsqueeze(1)), self.signs.unsqueeze(1)), dim=1
                ).unsqueeze(
                    1
                )  # (edge_attr, 1, 2, ...)
                dx = torch.sum(
                    weights * x_e, dim=2
                )  # the broadcasting might not work right here?
    ...
    def Laplacian(
        ...
        if self.signtype == "gradient":
            ...
            if len(edge_weights.shape) != len(dx.shape):
                edge_weights = edge_weights.reshape(
                    torch.cat(
                        [
                            edge_weights.shape,
                            torch.ones(
                                len(dx.shape) - len(edge_weights.shape), dtype=torch.int
                            ),
                        ]
                    )
                )
    ...
    def sheaf_diffusion(
        ...
        if self.adaptive_stepsize:
            alpha = torch.linalg.norm(x) / (
                torch.clamp(2 * torch.linalg.norm(Lx), min=1)
            )  # ensure ||Lx|| = 1/2 ||x||
        return x - alpha * Lx

    def edge_diffusion_gradient(
        self,
        x: torch.FloatTensor,
        edge_index: torch.LongTensor,
        ...
        gradient = torch.sum(dx_all * dx, dim=(2, 3)).permute(
            (1, 0)
        )  # (n_edge_features, edges)
        return gradient
    ...
    def edge_diffusion(
        ...
        if self.adaptive_stepsize:
            alpha = torch.linalg.norm(grad) / (
                torch.clamp(5 * torch.linalg.norm(edge_attr), min=1)
            )
        ...
        if self.layertype == "Laplacian":
            ...
            return (
                self.sheaf_diffusion(
                    x, edge_index, edge_attr, self.euler_stepsize_gradient
                ),
                edge_attr,
            )
            ...
            return (
                self.sheaf_diffusion(x, edge_index, edge_attr),
                self.edge_diffusion(x, edge_index, edge_attr),
            )
...
class SheafMap(torch.nn.Module):
    ...
    def __init__(
        ...
        self.affine = affine
        self.morphism = torch.nn.Parameter(
            torch.randn(out_stalk_dim, in_stalk_dim) / in_stalk_dim
        )
        self.feature_map = torch.nn.Parameter(
            torch.randn(in_feature_dim, out_feature_dim) / in_feature_dim
        )
        if self.affine:
            self.b = torch.nn.Parameter(
                torch.zeros(out_stalk_dim, out_feature_dim)
            )  # should this really be a matrix, or should it be uniform across one or both dimensions?
